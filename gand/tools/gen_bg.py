...
for name, t in THEMES.items():
    ...
    s = int(H * 0.86)
    canvas = over(canvas, ink, place(mask_scaled((s, s)), (s, s), W * 0.5, H * 0.47) * t["sigil"])
    ...
    canvas = over(canvas, ink, place(ring, (s, s), W * 0.80, H * 0.30) * t["ring"])
    s2 = int(H * 1.05)
    ring2 = mask_scaled((s2, s2)) * annulus(s2, 0.56, 0.65)
    canvas = over(canvas, stone, place(ring2, (s2, s2), W * 0.13, H * 0.95) * (t["ring"] * 0.85))
